...
class MLService:
    ...
    def predict_health(self, user_data: Dict[str, Any], log_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            screen_time = float(log_data.get("screen_time", 2) or 2)
            late_night_usage = float(log_data.get("late_night_usage", 0) or 0)
            
            #Protein tracking
            protein_intake = float(log_data.get("protein_intake",0) or 0)
            protein_goal = float(log_data.get("protein_goal",100) or 100)
            
            protein_percentage = 0
            if protein_goal > 0:
                protein_percentage = round((protein_intake / protein_goal) * 100, 1)
            ...
            return {
                ...
                "protein_tracking": {
                    ...
                    "remaining_protein": max(0, protein_goal - protein_intake),
                    "status": (
                        "Goal Achieved"
    ...
    def _generate_insights(self, log_data: Dict[str, Any], health_risk_score: float) -> Dict[str, Any]:
        ...
        if protein_goal > 0:
            protein_percent = (protein_intake / protein_goal) * 100
            
        if protein_percent < 50:
            recommendations.append(
                "Increase daily protein intake to support energy, muscle recovery, and overall health"    
            )
        elif protein_percent >= 100:
    ...
    def _map_supabase_to_model_input(self, log_data: Dict[str, Any], user_data: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        meals = int(log_data.get("meals_per_day", 3) or 3)
        cal = int(log_data.get("calorie_intake", 1800) or 1800)
        ...
        protein_ratio = 0
        if protein_goal > 0:
            protein_ratio =(protein_intake / protein_goal) * 100
    ...
    _FEATURE_SUGGESTIONS = {
        ...
        "ProteinIntake": "Increase protein-rich foods like eggs, paneer, fish, chicken, lentils, tofu, and nuts",
        "ProteinGoal": "Set a realistic daily protein goal based on your body weight and activity",
        "ProteinRatio": "Try to consistently meet your daily protein target",
